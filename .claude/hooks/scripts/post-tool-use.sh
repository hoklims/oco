...
# --- Timestamp-based modification/verification tracking ---
# Uses epoch seconds to resolve race conditions between concurrent Edit and Bash tool calls.
# Invariant: verify-done is valid only if its timestamp > last-modified timestamp.
...
if [ "$TOOL_NAME" = "Edit" ] || [ "$TOOL_NAME" = "Write" ] || [ "$TOOL_NAME" = "MultiEdit" ]; then
  ...
  if [ -n "$FILE_PATH" ]; then
    ...
    echo "$NOW" > "${OCO_STATE_DIR}/last-modified-ts" 2>/dev/null || true
  fi
fi
  ...
  case "$TOOL_NAME" in
    Bash|bash)
      ...
      if [ "$VERIFIED" = "true" ]; then
        echo "$NOW" > "${OCO_STATE_DIR}/verify-done" 2>/dev/null || true
      fi
      ;;
  esac
fi
