...
MODIFIED_TS_FILE="${OCO_STATE_DIR}/last-modified-ts"
if [ -f "$VERIFY_LOG" ] && [ ! -L "$VERIFY_LOG" ]; then
  VERIFY_TS=$(cat "$VERIFY_LOG" 2>/dev/null || echo "0")
  MODIFIED_TS=$(cat "$MODIFIED_TS_FILE" 2>/dev/null || echo "0")
  if [ "$VERIFY_TS" -ge "$MODIFIED_TS" ] 2>/dev/null; then
    # Verification timestamp >= last modification timestamp — clean up and allow stop
    rm -f "$MODIFIED_LOG" "$VERIFY_LOG" "$MODIFIED_TS_FILE" 2>/dev/null || true
    exit 0
  fi
  # Verification happened before last modification — fall through to block
fi
...
if [ -z "$NEEDS_CHECK" ]; then
  ...
  rm -f "$MODIFIED_LOG" "$MODIFIED_TS_FILE" 2>/dev/null || true
  exit 0
fi
